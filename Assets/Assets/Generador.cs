...
public class Generador : MonoBehaviour
{
    ...
    {                                 // generador de NPC
        cantbody = rn.Next(minimo, maximo);
                                        //-------------------- creacion de vampires ---------------\\
        for (int i = 0; i <cantbody; i++)
        {
            VampiroMesh = GameObject.CreatePrimitive(PrimitiveType.Cylinder);
            VampiroMesh.AddComponent<VampiroOP>();
            Vector3 poss = new Vector3(rn.Next(-20,20),0, rn.Next(-20,20));
            VampiroMesh.transform.position = poss;
            VampiroMesh.AddComponent<Rigidbody>();
            ZombieMesh.name ="vampi";
        }
